...
def Drawing(centroid_target, bounding_box_target, centroid_threat, bounding_box_threat):
    ...
    if dist_to_target_closest <= 100 and area_target > 6000:
        flag_target_close = True
        print('The target is close!')
        print('the area of the target is = ' + str(area_target))
    else:
        flag_target_close = False

    if dist_to_threat_closest <= 100 and area_threat > 6000:
        flag_threat_close = True
        print('The threat is close!')
...
def main():
    ...
    pub_setpoint = rospy.Publisher('/detection', camera_detection, queue_size=1)
